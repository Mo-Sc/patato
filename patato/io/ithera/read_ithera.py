    ...
    def _get_us_data(self):
        ...
        if len(us_files) > 0:
            ...
            if len(us_nodes) > 0:
                ...
                us_shape = (
                    int(us_pixel_counts.getElementsByTagName("Z")[0].firstChild.nodeValue),
                    int(us_pixel_counts.getElementsByTagName("Y")[0].firstChild.nodeValue),
                    int(us_pixel_counts.getElementsByTagName("X")[0].firstChild.nodeValue),
                )
                ...
                fov = (
                    float(us_extents[0].getElementsByTagName("Z")[0].firstChild.nodeValue),
                    float(us_extents[0].getElementsByTagName("Y")[0].firstChild.nodeValue),
                    float(us_extents[0].getElementsByTagName("X")[0].firstChild.nodeValue),
                )
            else:
                # Fallback if only single resolution is given
                us_pixels = int(
                    self.xml_tree.getElementsByTagName("ULTRA-SOUND-RESOLUTION")[0].firstChild.nodeValue
                )

                us_shape = (us_pixels, 1, us_pixels)

                pixel_size = float(
                    self.xml_tree.getElementsByTagName("UltraSoundPixelSize")[0].firstChild.nodeValue
                )
            ...
            try:
                us_data = np.memmap(us_files[0], mode="r", dtype=np.float32).reshape((-1, *us_shape))
                return us_data, fov
            except ValueError:
                print(
